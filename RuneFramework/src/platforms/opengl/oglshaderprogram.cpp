...
namespace Rune
{
    OGLShaderProgram::OGLShaderProgram(const char* shaderFilePath) : 
    ...
    bool OGLShaderProgram::CheckForCompileErrors(const unsigned int shader, unsigned int type) const
    {
        int  success;
        char infoLog[512];
    ...
    const int OGLShaderProgram::GetAndCacheUniform(const std::string& name) 
    {
    ...
    void OGLShaderProgram::SetVec2(const std::string& name, float x, float y)
    {
        int uniformLocation = GetAndCacheUniform(name);
        glUniform2f(uniformLocation, x, y);
    }

    void OGLShaderProgram::SetVec3(const std::string& name, float x, float y, float z)
    {
        int uniformLocation = GetAndCacheUniform(name);
        glUniform3f(uniformLocation, x, y, z);
    }

    void OGLShaderProgram::SetVec4(const std::string& name, float x, float y, float z, float w)
    {
        int uniformLocation = GetAndCacheUniform(name);
        glUniform4f(uniformLocation, x, y, z, w);
    }

    void OGLShaderProgram::SetVec2(const std::string& name, const glm::vec2& value)
    {
        int uniformLocation = GetAndCacheUniform(name);
        glUniform2fv(uniformLocation, 1, &value[0]);
    }

    void OGLShaderProgram::SetVec3(const std::string& name, const glm::vec3& value)
    {
        int uniformLocation = GetAndCacheUniform(name);
        glUniform3fv(uniformLocation, 1, &value[0]);
    }

    void OGLShaderProgram::SetVec4(const std::string& name, const glm::vec4& value)
    {
        int uniformLocation = GetAndCacheUniform(name);
        glUniform4fv(uniformLocation, 1, &value[0]);
    }

    void OGLShaderProgram::SetMat2(const std::string& name, const glm::mat2& mat)
    {
        int uniformLocation = GetAndCacheUniform(name);
        glUniformMatrix2fv(uniformLocation, 1, GL_FALSE, &mat[0][0]);
    }

    void OGLShaderProgram::SetMat3(const std::string& name, const glm::mat3& mat)
    {
        int uniformLocation = GetAndCacheUniform(name);
        glUniformMatrix3fv(uniformLocation, 1, GL_FALSE, &mat[0][0]);
    }

    void OGLShaderProgram::SetMat4(const std::string& name, const glm::mat4& mat)
    {
        int uniformLocation = GetAndCacheUniform(name);
